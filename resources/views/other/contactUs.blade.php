        ...
        <div class="row">
            <div class="col-md-8">
                <div class="well well-sm">
                    <form action="{{asset('/contact-us')}}" method="POST">
                        {{csrf_field()}}
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="name" name="tbFullName"
                                           placeholder="Enter name" required="required" value="{{old('tbFullName')}}"/>
                                </div>
                                <div class="form-group">
                                    <label for="email">
                                        Email Address</label>
                                  ...
                                  <span class="input-group-addon"><span class="glyphicon glyphicon-envelope"></span>
                                        ...
                                        <input type="email" class="form-control" id="email" name="tbEmail"
                                               placeholder="Enter email" required="required" value="{{old('tbEmail')}}"/></div>
                                </div>
                                <div class="form-group">
                                    <label for="subject">
                                        Subject</label>
                                    <select id="subject" class="form-control" name="soSubject"
                                            required="required">
                                        <option value="" @if (old('soSubject') == "") {{ 'selected' }} @endif>Choose One:</option>
                                        <option value="service" @if (old('soSubject') == "service") {{ 'selected' }} @endif >General Customer Service</option>
                                        <option value="suggestions" @if (old('soSubject') == "suggestions") {{ 'selected' }} @endif >Suggestions</option>
                                        <option value="product" @if (old('soSubject') == "product") {{ 'selected' }} @endif >Product Support</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <textarea id="message" class="form-control no-resize" rows="9"
                                              cols="25" name="tbMessage" required="required"
